...
const playlist = {
  ...
  update: function () {
    ...
    button.title =
      this.songQueue[last].title + " - " + this.songQueue[last].artist;
    let slogan = document.createElement("p");
    if (this.songQueue[last].artist != "")
      slogan.innerText =
        this.songQueue[last].artist + "  •  " + this.songQueue[last].time;
...
function initPage() {
  ...
  if (localStorage.getItem("color-scheme")) {
    document.getElementById("scheme-selector").value =
  ...
  if (location.host.indexOf(".netlify") != -1) fetchMeta.netlifyAPI = true;
      ...
      switch (radio) {
        case "last":
          if (localStorage.getItem("lastStation"))
            rSelect(localStorage.getItem("lastStation"));
          break;
        case "fav":
          if (favs.length > 0) rSelect(favs[0]);
          break;
        default:
          if (r.has(radio)) rSelect(radio);
      }
    }
  }
}
  ...
  if (favs.length !== 0) {
    ...
    for (i = 0; i < list.length; i++) {
      if (list[i].getAttribute("data-radioid") === np) {
        if (i === list.length - 1)
          rSelect(list[0].getAttribute("data-radioid"));
...
function prevStation() {
  if (favs.length !== 0) {
    ...
    for (i = 0; i < list.length; i++) {
      if (list[i].getAttribute("data-radioid") === np) {
        if (i === 0)
          rSelect(list[list.length - 1].getAttribute("data-radioid"));
        else rSelect(list[i - 1].getAttribute("data-radioid"));
        break;
      }
    }
...
function rSelect(selected) {
          ...
          if (r.get(selected).audio.slice(0, 5) === "http:") {
            ...
            setTimeout(function () {
              window.open(
                ...
                "targetWindow",
                "toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=yes,height=100px,width=400px"
              );
            }, 2000);
  ...
  document.getElementById("big-logo").alt =
    r.get(selected).name + " logo / Album artwork";

  np = selected;
  localStorage.setItem("lastStation", selected);

  if (radiotitle.offsetWidth < radiotitle.scrollWidth)
    document.getElementById("openWebsite-button").style.display = "none";
...
function openPage() {
  if (!np) return;
  if (location.origin.slice(-2) == "pp")
    window.open(r.get(np).website, "_blank");
...
function shareAPI() {
  ...
  if (np) {
    ...
    data.url =
      "https://simpleradio.netlify.app" + location.pathname + "?s=" + np;
    let now = document.getElementById("song-info").innerText;
    if (now != "")
      data.text = text.sharing[0] + r.get(np).name + text.sharing[1] + now;
  }
  ...
  fetch("/.netlify/functions/fetchMeta?s=" + selected)
    ...
    .then(function (rsp) {
      ...
      if (!fetchMeta.metaInterv) {
        fetchMeta.metaInterv = setInterval(function () {
          fetchMetadata(np);
        }, 30000);
      }
      switch (selected) {
        ...
        case "dancefm": {
          ...
          if (
            playlist.songQueue.length &&
            tmp.includes(
              playlist.songQueue[playlist.songQueue.length - 1].artist
            )
          )
            return;
          break;
        ...
        case "onefm": {
          ...
          if (text.ArtistName != "")
            tmp = text.ArtistName + " - " + text.TrackTitle;
        ...
        case "bestfm": {
          let text = JSON.parse(rsp.result);
          if (text.player[0].artist != "")
            tmp = text.player[0].artist + " - " + text.player[0].title;
        ...
        case "europafm": {
          let text = JSON.parse(rsp.result);
          if (text.songs[0].artist != "")
            tmp = text.songs[0].artist + " - " + text.songs[0].track;
...
function getMetadata(selected) {
    ...
    if (stats.icestats != undefined) {
      if (stats.icestats.source.hasOwnProperty(length)) {
        for (let i = 0; i < stats.icestats.source.length; i++) {
          let slash =
            r.get(selected).audio.lastIndexOf("/") -
            r.get(selected).audio.length;
          if (
            stats.icestats.source[i].listenurl.slice(slash) ==
            r.get(selected).audio.slice(slash)
          )
            titleTmp = stats.icestats.source[i].title;
        }
  ...
  ice = new IcecastMetadataStats(r.get(selected).audio, {
    onStats: onStatsFunc,
    sources: ["icy", "icestats"],
  });
  ...
  ice.getIcestats().then(function (rsp) {
    if (rsp.icestats != undefined) {
      ...
      ice.getIcyMetadata().then(function (rsp) {
...
function formatMetadata(data) {
  ...
  let time =
    currentTime.getHours() + ":" + (minutes < 10 ? "0" + minutes : minutes);
  if (minus != -1) {
...
function getArtwork(title, artist) {
  fetch("/.netlify/functions/getArtwork?a=" + artist + "&t=" + title)
    ...
    .then(function (response) {
      document
        .getElementById("song-link")
        .setAttribute("href", response.result.album.url);
      let url = JSON.stringify(response.result.album.image[3]).substring(
        30,
        JSON.stringify(response.result.album.image[3]).length - 2
      );
      if (url != "") {
...
function timeInput() {
  ...
  if (str === "") {
    ...
    toastMessage(
      text.timer[0] + Math.floor(ido / 60) + ":" + perc + text.timer[1]
    );
  }
}
...
function settingsInit() {
  ...
  player.addEventListener("pause", function () {
    ...
    if (document.getElementById("aod-play"))
      document.getElementById("aod-play").innerText = "play_arrow";
  ...
  player.addEventListener("playing", function () {
    ...
    if (document.getElementById("aod-play"))
      document.getElementById("aod-play").innerText = "pause";
    document.title = r.get(np).name + " - " + text.title;
    if (document.getElementById("metadata").checked) getMetadata(np);
    if (
      (document.getElementById("wake-lock").checked && !wLock) ||
      document.getElementById("aod")
    ) {
      try {
        navigator.wakeLock
          .request("screen")
    ...
    function (event) {
      ...
      if (!settings.style.transform) {
        focusable = Array.prototype.slice.call(
          document.getElementsByClassName("focusable-setting")
        );
        focusable.unshift(document.getElementById("settings-button"));
      } else {
        if (document.getElementById("aod")) {
          focusable = Array.prototype.slice.call(
            document.getElementsByClassName("aod-x")
          );
          focused = 0;
        } else {
          focusable = Array.prototype.slice.call(
            document.getElementsByClassName("focusable")
          );
  ...
  document.getElementById("contentID").addEventListener("scroll", function () {
    if (document.getElementById("contentID").scrollTop < 400)
      document.getElementById("scroll-up").style.display = "none";
    else document.getElementById("scroll-up").style.display = "block";
  });
  caches
    ...
    .then(function (rsp) {
      ...
      devData.innerText +=
        "Version: " + rsp[0].match(/[0-9]+\.[0-9]+(\.[0-9]+)?/)[0];
...
function batteryGen(level) {
  ...
  return (
    '<span class="material-icons" style="font-size:inherit; vertical-align:sub;"> ' +
    txt +
    " </span>"
  );
}
...
function aod() {
  ...
  xButton.onclick = function () {
    ...
    titleContainer.insertBefore(
      document.getElementById("radio-title"),
      titleContainer.firstChild
    );
    document
      .getElementById("big-logo-container")
      .appendChild(document.getElementById("big-logo"));
    document
      .getElementById("song-link")
      .appendChild(document.getElementById("song-info"));

    clearInterval(clockInterval);
    if (
      !player.paused &&
      wLock &&
      !document.getElementById("wake-lock").checked
    ) {
      wLock
        .release()
        .then(function () {
  ...
  try {
    ...
    if (castSession && np) {
      ...
      mediaInfo.metadata.images = [
        { url: "https://simpleradio.netlify.app/img/stations/" + np + ".png" },
        { url: "https://simpleradio.netlify.app/img/stations/0custom.png" },
      ];
      ...
      castSession.loadMedia(request).then(
        ...
        function (errorCode) {
          console.log("Cast Error code: " + errorCode);
        }
      );
      player.pause();
    }
